...
2. Ask: **"Which data module? (e.g. `weather`, `user`)"**
    ...
    - If it **does not exist**: invoke the `/feature-scaffold` skill to create the module first.
...
```bash
python3 .claude/skills/datastore-cache-scaffold/scripts/generate_cache.py \
  --name <Name> \
  --data-module <module> \
