...
This project strictly enforces a **Namespace-Driven, Layered Feature Architecture**.
You MUST group modules into physical directories that match their Gradle namespace.

## 1. Directory Separation (Keep Root Clean)
* **FORBIDDEN:** Do not mix source code modules with project configuration files in the root directory.
* Physical folders like `domain/`, `data/`, `feature/`, and `core/` act as organizational root folders for the modules.
* Example: A module with the namespace `:domain:weather` MUST live in the physical directory `domain/weather/`.
...
* Example for `:domain:weather`: `package com.example.myapp.domain.weather`
...
### A. `:app` (The Assembler)
* **Location:** `app/`
* **Role:** Only contains the Application class, global DI initialization, and the root Navigation Graph mapping.
...
* **Location:** `domain/[feature]/` (e.g., `domain/search`, `domain/weather`)
...
* **Location:** `data/[feature]/` (e.g., `data/search`, `data/weather`)
* **Role:** Contains API implementations (Ktor), DTOs, Room Entities, DAOs, and Repository implementations.
* **Rule:** MUST implement interfaces defined in its respective `:domain:[feature]` module.

### E. `:feature:[feature]` (UI & Presentation)
* **Location:** `feature/[feature]/` (e.g., `feature/search`, `feature/weather`)
* **Role:** Contains ViewModels, UI State, and Jetpack Compose screens.
* **Rule:** MUST NOT contain any network DTOs or database entities. Uses models from `:domain:[feature]`.
...
4. **FORBIDDEN:** A `:feature` module MUST NEVER depend on another `:feature` module. (Use deep links or route registries in `:app`).
...
Creating the module is only half the job. Inside each module's `src/main/kotlin/com/[company]/[app]/[layer]/[feature]/` directory, you MUST organize files into specific sub-packages based on their responsibility.
...
* `.mapper` - Extension functions converting DTOs/Entities to Domain models (e.g., `SearchResultsToDomain.kt`).
...
* `.ui.component` - Small, reusable UI elements specific to this feature (can be grouped further, e.g., `.ui.component.header`).
* `.navigation` - NavGraph builder extensions and route objects.

## 6. Module Creation Rules
When asked to "create a module":
1. Create the physical directory structure matching the namespace (e.g., `mkdir -p data/weather/src/main/kotlin/...`).
2. Add a `build.gradle.kts` using Kotlin DSL.
3. Register the namespace in the root `settings.gradle.kts` (e.g., `include(":data:weather")`).
