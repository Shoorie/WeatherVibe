...
class HomeStateFactoryTest {
  ...
  private val getCurrentWeatherMetrics = mockk<GetCurrentWeatherMetrics>()
  ...
  @Before
  fun setUp() {
    every { getCurrentWeatherMetrics(any()) } returns METRICS
  ...
  fun `given loaded state with playlist, when temperatures reformatted, then preserve playlist`() {
    ...
    val playlist = playlistFactory.create(com.weather.vibe.testing.weather.fixture.WeatherSuggestionFixtures.SUGGESTION)
    val created = factory.create(WEATHER)
