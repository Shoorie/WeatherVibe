...
internal object LocationsKeys {
  ...
  fun card(favoriteId: Long): String = "$CARD_PREFIX$favoriteId"

  fun isCard(key: Any?): Boolean = (key as? String)?.startsWith(CARD_PREFIX) == true

  fun favoriteIdFromCardKey(key: Any?): Long? =
    (key as? String)?.removePrefix(CARD_PREFIX)?.toLongOrNull()
}
