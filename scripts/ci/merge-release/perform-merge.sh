...
# Files each release branch keeps its own copy of, so a conflict on them is
# always resolved to the target's side. Add a path here to auto-resolve it too.
OWNED_BY_EACH_RELEASE=(version.txt dependencies.gradle.kts)
...
use_bot_identity() {
    local bot_email="41898282+github-actions[bot]@users.noreply.github.com"
    git config user.name "${GIT_USER_NAME:-github-actions[bot]}"
    git config user.email "${GIT_USER_EMAIL:-$bot_email}"
}
...
merge_source_into_merge_branch() {
    local message="Merge branch '$SOURCE_BRANCH' into $MERGE_BRANCH"
    git merge --no-ff -m "$message" "origin/$SOURCE_BRANCH" \
        && return 0
...
is_owned_by_each_release() {
    local owned
    for owned in "${OWNED_BY_EACH_RELEASE[@]}"; do
        [[ "$1" == "$owned" ]] && return 0
    done
    return 1
}
...
keep_target_submodule_revision() {
    ...
    target_entry=$(git ls-files -u -- "$path" \
        | awk '$3 == "2" { print $1 "," $2; exit }')
